// Copyright (C) 2024 - present, MissingCore
// SPDX-License-Identifier: AGPL-3.0-only

import type { ClickActionProps, HexColor } from "react-native-android-widget";
import { SvgWidget } from "react-native-android-widget";

const SVGMap: Record<string, (color: HexColor) => string> = {
  play: (color) => `
    <svg width="100%" height="100%" viewBox="0 -960 960 960" fill="${color}">
      <path d="M345.66-307.55v-345.09q0-15.13 10.31-24.57t24-9.44q4.34 0 9 1.06 4.67 1.05 9.02 3.58l271.12 173.39q7.73 5.44 11.7 12.82 3.96 7.38 3.96 15.8 0 8.42-3.96 15.8-3.97 7.38-11.7 12.62L397.99-278.19q-4.37 2.73-9.05 3.78-4.67 1.06-8.77 1.06-13.94 0-24.23-9.44-10.28-9.44-10.28-24.76Z" />
    </svg>
  `,
  pause: (color) => `
    <svg width="100%" height="100%" viewBox="0 -960 960 960" fill="${color}">
      <path d="M615.65-226.46q-23.05 0-39.5-16.55-16.46-16.54-16.46-39.41v-395.16q0-22.87 16.46-39.41 16.45-16.55 39.5-16.55h34.04q22.88 0 39.42 16.55 16.54 16.54 16.54 39.41v395.16q0 22.87-16.54 39.41-16.54 16.55-39.42 16.55h-34.04Zm-305.15 0q-23.05 0-39.51-16.55-16.45-16.54-16.45-39.41v-395.16q0-22.87 16.45-39.41 16.46-16.55 39.51-16.55h34.04q22.87 0 39.42 16.55 16.54 16.54 16.54 39.41v395.16q0 22.87-16.54 39.41-16.55 16.55-39.42 16.55H310.5Z" />
    </svg>
  `,
  prev: (color) => `
    <svg width="100%" height="100%" viewBox="0 -960 960 960" fill="${color}">
      <path d="M251.89-303.04v-354.2q0-11.35 8.22-19.58 8.23-8.22 19.77-8.22t19.76 8.24q8.21 8.23 8.21 19.65v354.11q0 11.63-8.24 19.86-8.24 8.22-19.66 8.22-11.64 0-19.85-8.22-8.21-8.23-8.21-19.86Zm403.22-7.15L443.59-451.67q-7.78-5.14-11.55-12.56-3.77-7.42-3.77-15.81 0-8.38 3.77-15.77 3.77-7.38 11.55-12.53L655.11-650q4.43-3.11 9.2-4.36 4.77-1.25 9.77-1.25 13.46 0 23.75 9.63 10.28 9.64 10.28 24.99v281.87q0 15.45-10.28 24.99-10.29 9.55-23.75 9.55-5 0-9.77-1.15-4.77-1.16-9.2-4.46Z" />
    </svg>
  `,
  next: (color) => `
    <svg width="100%" height="100%" viewBox="0 -960 960 960" fill="${color}">
      <path d="M652.15-303.04v-354.2q0-11.35 8.23-19.58 8.22-8.22 19.77-8.22 11.54 0 19.75 8.24 8.21 8.23 8.21 19.65v354.11q0 11.63-8.24 19.86-8.23 8.22-19.65 8.22-11.64 0-19.86-8.22-8.21-8.23-8.21-19.86Zm-400.26-36.08v-281.87q0-15.35 10.27-24.99 10.26-9.63 23.96-9.63 4.8 0 9.57 1.25 4.77 1.25 9.2 4.36l211.52 141.66q7.78 5.15 11.55 12.57 3.77 7.42 3.77 15.81 0 8.38-3.77 15.77-3.77 7.38-11.55 12.52L304.89-310.19q-4.43 3.3-9.2 4.46-4.77 1.15-9.57 1.15-13.7 0-23.96-9.55-10.27-9.54-10.27-24.99Z" />
    </svg>
  `,
  placeholder: (color) => `
    <svg width="100%" height="100%" viewBox="0 0 450 450" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="312" cy="347" r="15" fill="${color}" />
      <circle cx="277" cy="347" r="15" fill="${color}" />
      <circle cx="137" cy="347" r="15" fill="${color}" />
      <circle cx="102" cy="347" r="15" fill="${color}" />
      <circle cx="347" cy="312" r="15" fill="${color}" />
      <circle cx="312" cy="312" r="15" fill="${color}" />
      <circle cx="277" cy="312" r="15" fill="${color}" />
      <circle cx="172" cy="312" r="15" fill="${color}" />
      <circle cx="137" cy="312" r="15" fill="${color}" />
      <circle cx="102" cy="312" r="15" fill="${color}" />
      <circle cx="347" cy="277" r="15" fill="${color}" />
      <circle cx="172" cy="277" r="15" fill="${color}" />
      <circle cx="347" cy="242" r="15" fill="${color}" />
      <circle cx="172" cy="242" r="15" fill="${color}" />
      <circle cx="347" cy="207" r="15" fill="${color}" />
      <circle cx="172" cy="207" r="15" fill="${color}" />
      <circle cx="347" cy="172" r="15" fill="${color}" />
      <circle cx="312" cy="172" r="15" fill="${color}" />
      <circle cx="277" cy="172" r="15" fill="${color}" />
      <circle cx="242" cy="172" r="15" fill="${color}" />
      <circle cx="207" cy="172" r="15" fill="${color}" />
      <circle cx="172" cy="172" r="15" fill="${color}" />
      <circle cx="347" cy="137" r="15" fill="${color}" />
      <circle cx="312" cy="137" r="15" fill="${color}" />
      <circle cx="277" cy="137" r="15" fill="${color}" />
      <circle cx="242" cy="137" r="15" fill="${color}" />
      <circle cx="207" cy="137" r="15" fill="${color}" />
      <circle cx="172" cy="137" r="15" fill="${color}" />
      <circle cx="347" cy="102" r="15" fill="${color}" />
      <circle cx="312" cy="102" r="15" fill="${color}" />
      <circle cx="277" cy="102" r="15" fill="${color}" />
      <circle cx="242" cy="102" r="15" fill="${color}" />
      <circle cx="207" cy="102" r="15" fill="${color}" />
      <circle cx="172" cy="102" r="15" fill="${color}" />
    </svg>
  `,
} as const;

export type WidgetSVGName = keyof typeof SVGMap;

/** Render one of the pre-defined SVGs. */
export function WidgetSVG({
  name,
  size,
  color,
  ...props
}: ClickActionProps & { name: WidgetSVGName; size: number; color: HexColor }) {
  return (
    <SvgWidget
      svg={SVGMap[name]!(color)}
      style={{ height: size, width: size }}
      {...props}
    />
  );
}
